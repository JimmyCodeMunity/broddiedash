...
const PendingOrders = () => {
    const { pendingorders,updateOrder } = useContext(ApiContext)
    // console.log("porder",pendingorders)
                                            ...
                                            pendingorders?.map((porder) => {
                                                return (
                                                    <tr class="hover:bg-gray-100">
                                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800"><img src={`https://smokey.haramad.co.ke/${porder?.mealid?.image}`} alt="" className="h-10 w-10 rounded-full" /></td>
                                                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-800">{porder?.mealid?.mealname}</td>
                                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">Kshs.{porder?.mealid?.price}</td>
                                                        <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">{porder?.userid?.email}</td>
                                                        <td class="px-6 py-4 whitespace-nowrap text-end text-sm font-medium space-x-2">
                                                            <button onClick={()=>updateOrder(porder?._id,action)} type="button" class="inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent text-orange-600 hover:text-orange-500 ">Complete</button>
