...
const Meals = () => {
  const { meals, getAllArt, updateFeatured, updatePopular } =
                    ...
                    {meals.map((meal) => {
                      return (
                        <tr class="hover:bg-gray-100">
                          ...
                          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                            <img
                              src={`https://server.broddiescollection.com/${meal?.image}`}
                              alt=""
                          ...
                          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                            <ReactSwitch
                              onChange={
                                meal.featured === "false"
                                  ? () => updateFeatured(meal._id, active)
                                  : () => updateFeatured(meal?._id, inactive)
                              }
                              checked={
                                meal?.featured === "false" ? false : true
                              }
                            />
                          </td>
                          <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-800">
                            <ReactSwitch
                              onChange={
                                meal.popular === "false"
                                  ? () => updatePopular(meal._id, active)
                                  : () => updatePopular(meal?._id, inactive)
                              }
