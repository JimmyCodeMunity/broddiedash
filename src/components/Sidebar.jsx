...
import { ChefHat, CircleCheck, ClipboardCheck, ClockArrowDown, LayoutList, Loader } from "lucide-react";
...
export const sidebarLinks = [
    ...
    },
    {
        ...
        icon: ClockArrowDown,
        actions: [
            { id: 1, title: "Pending Orders", path: "/pendingorders/view", icon: Loader },
            { id: 2, title: "Completed Orders", path: "/completedorders/view", icon: CircleCheck },
...
export default function Sidebar() {
    ...
    return (
        <div className={`${minimenu ? "md:w-[6%]" : "md:w-[20%]"} w-full h-screen md:block hidden bg-white p-4 space-y-10`}>
            <div className="w-full flex flex-row justify-between items-center">
                {
                    !minimenu && <div className="w-full py-5 flex flex-row items-end space-x-4 px-4">
                    <img src="../broddie.jpeg" className="h-12 w-12 rounded-full" alt="Haus Logo" />
                    <h1 className="text-black font-bold tracking-wider text-xl">Broddie's Collection</h1>
                </div>
                }

                {/* <div className="py-5">
                    <Link onClick={showIcons}>
                        ...
                        <li key={link.id} className="mb-2">
                            ...
                            >
                                <Link to={link.path} className="flex items-center gap-2 w-full text-neutral-500">
                                    <link.icon size={20} />
                                    {
                                        !minimenu && <span className={`${isActive && "text-white"} text-neutral-700`}>{link.title}</span>
                                    }
                                </Link>
                                {link.actions && (
                                    <>
                                        {
                                            !minimenu && <ChevronDown
                                                size={18}
                            ...
                            {link.actions && (
                                <motion.ul
                                    ...
                                    className="ml-6 mt-1 border-l border-orange-300 pl-3 overflow-hidden space-y-2"
                                >
                                    {link.actions.map((action) => (
                                        <li key={action.id}>
                                            <Link
                                                to={action.path}
                                                className={`flex flex-row space-x-3 w-full p-2 rounded-md transition ${location.pathname === action.path ? "bg-orange-500 text-white" : "hover:bg-gray-200"
                                                    }`}
                                            >
                                                {action.icon && <action.icon className="text-neutral-500" size={20} />}
                                                <span className="text-neutral-500 text-sm">{action.title}</span>
                                            </Link>
                                        </li>
                                    ))}
                                </motion.ul>
